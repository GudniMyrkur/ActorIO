...
void UActorIOLibrary::K2_RegisterIOEvent(UObject* WorldContextObject, FActorIOEventList& Registry, FName EventId,
                                         const FText& DisplayNameText, const FText& TooltipText,
                                         FName EventDispatcherName, FName EventProcessorName, FName SubobjectName)
{
	...
	if (AActor* DelegateOwnerActor = Cast<AActor>(DelegateOwner))
	{
		for (auto Component : DelegateOwnerActor->GetComponents())
		{
			if (Component->GetName() == SubobjectName)
			{
				DelegateOwner = Component;
				break;
			}
		}
	}
	else if (IsValid(WorldContextObject) && !SubobjectName.IsNone())
	{
...
void UActorIOLibrary::K2_RegisterIOFunction(UObject* WorldContextObject, FActorIOFunctionList& Registry,
                                            FName FunctionId, const FText& DisplayNameText, const FText& TooltipText,
...
void UActorIOLibrary::K2_SetNamedArgument(UObject* WorldContextObject, const FString& ArgumentName,
